...
Here is a few examples on the different types of relationships
...
Once your models.py file is ready, you have to `migrate` and `upgrade` so you can synch the changes into your database engine.

### Creating migrations

This command will create all the migrations files in your `./migrations` folder, that way we have have them commited into github and everyone
...
> Note: It's important to mention that the `migrate` command does not update your database, you will have to `upgrade` if you want to actually synch changes to your database.
...
You will encounter lot's of errors when updating and migrating your database. 
This is were that SQL syntax knowledge comes handy.
