...
When using express.js those endpoints have to be added into your API be using the router.get (or post, put or delete) function. For example:

```js
router.get("/users", getUser)
```

The line above, specifies a new endpoint that other devs will be able to call by requestion `GET /users`.
...
We two files and you should should update one or the other **for each endpoint your create**:

- `public_routes.ts` is for API urls that are going to be used by anyone, no secuirt whatsoever, for example: Everyone can signup, everyone can try to login, etc.
- `private_routes.ts` this URLs will be only for logged in users, for example: Get my list of favorites, or get my information, etc.

Open your chosen `./src/<private|public>_routes.ts` file and add a new route to the list of entpoints, for example, if we want to build an endpoint to retrieve a single user information by a given ID, for example: `GET /user/2`
...
Junior developers always asume that everything will be O.K. Senior developers are prepared for the worst possible scenarios.
...
There are three possible types of validations that we recomend doing:

### A) Validating the request payload (body)

The request payload can be retrived by doing `req.body`, this is an example on how to validate if the request body contains the property `first_name`:
...
If your endpoint url is expeting a parameter you can access it by typing `req.params`, for example, if the endpoint `GET /user/:id` is called with `GET /user/23`, we can retrieve the value like this:
...
We strongly recomend using express-jwt and jsonwebtoken libraries for autentication.

The authentication can be split in 4 steps and [🔥🔥🔥 here is a very detailed explanation on how to implement it with express and typescript.](/other/express/jwt-authentication.md)
...
Asuming you want to delete user with ID=1
...
Asuming you want to update the user with the ID=1 and set his/her name to Rizzrak:
...
2. Apply your all of your pending migrations:
