...
# Build advanced applications with React
...
Make sure to follow the steps on [running the project](Running the project) first, then come back to this list, if you are using Gitpod or Codespaces it is possible that the project is already running.

### Add your first view

It's time to add some code: Everything starts at the application layout (`js/layout.js`), this file is like a table of contents were all of your applications pages will be added before they can be rendered by react and the browser, you can [read more about React Router here](https://4geeks.com/lesson/routing-our-views-with-react-router).

We already added a few routes that will like `Home`, `Demo` and `Single`, this views are useful examples of the most frequent things you usually need in a project.

All of you application pages will be added into the `js/pages` folder, each of them will be a separate react component.

Let's open the `<Home>` view at `js/pages/home.js`.

### Code your first view HTML

As you can see, the `Home.js` page is just a react component, here is a additional things to notice:

- The component returns HTML.
- At the of the component file (Home.js) there are two imports: The `AppContext` that will be used for dealing with any global information needed form other views or the application; And the `home.css` that will be used to add any CSS classes and styles used on this view in particular.
- It's also important to mention that the AppContext is declared inside the compoent, on the very first couple of lines:
...
- Use your class by adding the `<div className="background-blue">` into your page html at `js/pages/home.js`.

### Creating your first component

Usually you want most of your application html to be split into components that can be re-used.
...
For example, if we want to add a new `<Card>` component that replicates the [classic boostrap card](https://getbootstrap.com/docs/5.0/components/card/), we can create a `js/components/Card.js` with the following people:
...
Now that we have the `Card` component we can incorporated to our `Home.js` page with the following steps:
